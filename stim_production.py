...
for iblock in range(params.nblocks):
    ...
    specific_stimuli_dir = os.path.join(stimuli_dir, f"tapping_experiment_block{block_id}")
    os.makedirs(specific_stimuli_dir, exist_ok=True)

    # store sound signals 
    signal_dir = os.path.join(data_dir, f"tapping_experiment_block{block_id}", 'signal_data')
    ...
    for index, row in table_values.iterrows():
        ...
        stim_name = 'tapping_experiment_index_' + str(index) + '_unitdur_' + str(unitdur) + '_percentage_' + str(percentage) + '.wav'
    ...
    # save the data frame
    tsv_filename = 'tapping_experiment_block_' + str(block_id) + '_table.tsv'    
    ...
    signal_filename = 'tapping_experiment_block_' + str(block_id) + '_signal.pkl'
